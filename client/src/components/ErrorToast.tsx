...
import type { ToastState, ToastType } from '../hooks/useErrorToast';
...
export default function ErrorToast({ toast, onDismiss }: ErrorToastProps) {
  const [isExiting, setIsExiting] = useState(false);
  const styles = typeStyles[toast.type];

  useEffect(() => {
    // Auto-dismiss after 5 seconds if no retry function
    if (!toast.retryFn) {
  ...
  }, [toast.id, toast.retryFn, onDismiss]);
